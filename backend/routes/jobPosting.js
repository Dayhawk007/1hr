...
router.get("/", async (req, res) => {
  try {
    ...
    if (minCompensation)
      filter.compensationStart = { $gte: Number(minCompensation) };
    if (maxCompensation)
      filter.compensationEnd = { $lte: Number(maxCompensation) };
    if (minExperience)
      filter["experienceRange.min"] = { $gte: Number(minExperience) };
    if (maxExperience)
      filter["experienceRange.max"] = { $lte: Number(maxExperience) };
...
router.post(
  ...
  [
    ...
    body("compensationStart")
      .isNumeric()
      .withMessage("Compensation start must be a number"),
    body("compensationEnd")
      .isNumeric()
      .withMessage("Compensation end must be a number"),
    ...
    body("questions.*.questionType")
      .isIn(["text", "multiple-choice"])
      .withMessage("Invalid question type"),
    body("questions.*.options")
      .optional()
      .isArray()
      .withMessage("Options must be an array"),
    body("experienceRange.min")
      .isNumeric()
      .withMessage("Minimum experience must be a number"),
    body("experienceRange.max")
      .isNumeric()
...
router.get("/:jobId/applications", async (req, res) => {
  try {
    ...
    const { subVendorId } = req.query || "";

    let query = { job: jobId };

    // If subVendorId is provided, add it to the query filter
    if (subVendorId) {
      query.subVendor = subVendorId;
    }
    ...
    const applications = await Application.find(query)
      ...
      .populate("job", "title")
      .populate("subVendor", "name"); // Populate the job field with the job's title
